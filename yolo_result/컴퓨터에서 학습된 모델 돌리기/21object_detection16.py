...
def main():  
    try:
        while True:  
            ret, frame_bgr = cap.read()#카메라에서 프레임 받아오기
            if not ret:#못받으면 break
                break

            orig_h, orig_w = frame_bgr.shape[:2] #추후 시각화 위해 원본 저장
            
            inp_arr = preprocess(frame_bgr) #모델 입력 위한 전처리
            
            interpreter.set_tensor(inp['index'], inp_arr)  #모델의 입력에 전처리한거 넣기
            interpreter.invoke() #모델 실행
            prediction = interpreter.get_tensor(out['index']) #추론결과 저장

            detections = yolo_postprocess(#후처리 단계(임계값 이상만, 중복 박스 제거 좌표 추출)
                prediction, orig_w, orig_h,
                conf_thres=0.45, iou_thres=0.45
            ) 
        
            for (x1, y1, x2, y2), sc, cid in detections:#박스 하나씩 시각화
                cid = int(cid)
                ...
                # 바운딩 박스 및 라벨 그리기 (frame_rgb 그대로 사용 → 색감 그대로)
            ...
            cv2.imshow("TFLite Object Detection (Webcam)", frame_bgr)  
